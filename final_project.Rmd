...
When looking at our visualizations for the features, it appears that are is no significant difference between their distributions, as they are all right-skewed. This is supported by our results from the KS test, as the test statistics for each test are greater than the alpha-value of 0.05. As a result, it is reasonable to conclude that these features affect the value of `EP_UNEMP`. 
...
advanced_df$city_density <- advanced_df$E_TOTPOP / advanced_df$AREA_SQMI
# Fit multiple regression linear model
adv_model <- lm(EP_PCI ~ EP_UNEMP + E_TOTPOP + city_density, data = advanced_df)
```
